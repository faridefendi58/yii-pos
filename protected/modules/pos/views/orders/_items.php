...
if(Yii::app()->user->hasState('items_belanja')):
	...
	foreach(Yii::app()->user->getState('items_belanja') as $index=>$data){?>
		...
		<tr class="<?php echo($no%2>0)? 'even':'odd';?>">
			...
			<td style="text-align:right;" id="unit-price-<?php echo $index;?>"><?php echo number_format($data['unit_price'],0,',','.');?></td>
			<td style="text-align:center;"><?php echo CHtml::textField('qty',$data['qty'],array('size'=>3,'class'=>'text-center','onchange'=>'pushQty('.$index.',this,"'.Yii::app()->createUrl('/'.Yii::app()->controller->module->id.'/orders/updateQty').'")'));?></td>
			<td style="text-align:right;" id="discount-<?php echo $index;?>" class="overwrite hidden">
