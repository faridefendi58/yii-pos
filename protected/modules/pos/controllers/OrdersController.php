...
class OrdersController extends EController
{
    ...
    public function accessRules()
    {
        return array(
            array('allow',
                'actions' => array('create', 'suggestItems', 'scan', 'updateQty', 'paymentRequest', 'changeRequest', 'deleteItem', 'cancelTransaction', 'addmodal'),
                //'users'=>array('@'),
    ...
    public function actionSuggestItems()
    {
        if (isset($_GET['q']) && ($keyword = trim($_GET['q'])) !== '') {
            ...
            if(is_array($items))
                echo implode("\n",$items);*/
            $sql = "SELECT CONCAT(barcode,' - ',name) AS item FROM tbl_product WHERE barcode LIKE '%" . $keyword . "%' OR name LIKE '%" . $keyword . "%' OR tag REGEXP '" . $keyword . "' LIMIT 10";
